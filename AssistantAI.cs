...
namespace BaseAssistant
{
    public class AssistantAI : MonoBehaviour
    {
        ...
        private void ProcessAI()
        {
            ...
            else if (_currentTask == "MoveToChestToFetch") ExecuteFetchFromChest();
        ...
        private void FindNewTask()
        {
            ...
            // PRIORIDADE 4: Abastecer os Fornos (Carvoaria) respeitando o limite da config
        ...
        {
            string targetName = targetItem.m_shared.m_name?.ToLower() ?? "";
            ...
            if (!string.IsNullOrEmpty(customName))
            {
                ...
                string localizedName = Localization.instance.Localize(targetName).ToLower().Trim();
        ...
        private void ExecuteFetchFromChest()
        {
            float d = Plugin.ChestDistance.Value;
            if (IsCloseEnough(_targetObj, d, d))
            {
                ...
                if (chest != null && chest.GetInventory() != null)
                {
                    // Pega a referência do item pra saber o m_shared.m_name
                    GameObject prefab = PrefabManager.Cache.GetPrefab<GameObject>(_holdingItem); // _holdingItem aqui é a madeira que ele decidiu buscar
                    if (prefab != null)
                    {
                        string localizedName = prefab.GetComponent<ItemDrop>().m_itemData.m_shared.m_name;
                        int currentWood = chest.GetInventory().CountItems(localizedName);
                        
                        if (currentWood > 10)
                        {
                            int amountToTake = Mathf.Min(10, currentWood - 10);
                            ...
                            {
                                Smelter smelter = hit.GetComponentInParent<Smelter>();
                                if (smelter != null && smelter.name.ToLower().Contains("kiln") && smelter.GetComponent<ZNetView>().GetZDO().GetInt("queued", 0) < smelter.m_maxOre)
                                {
                                    SetTask("FeedKiln", smelter.gameObject);
                                    return;
                                }
                            }
                        }
        ...
        private void ExecuteFeedKiln()
        {
            float d = Plugin.KilnDistance.Value;
            if (IsCloseEnough(_targetObj, d, d))
            {
                Smelter kiln = _targetObj.GetComponent<Smelter>();
                if (kiln != null)
                {
                    ZNetView kilnView = kiln.GetComponent<ZNetView>();
                    int queued = kilnView.GetZDO().GetInt("queued", 0);
                    
                    int spaceLeft = kiln.m_maxOre - queued;
                    int amountToFeed = Mathf.Min(spaceLeft, _holdingAmount);
                    
                    if (amountToFeed > 0)
                    {
                        kilnView.ClaimOwnership();
                        
                        for(int i = 0; i < amountToFeed; i++)
                        {
                            kilnView.GetZDO().Set("item" + (queued + i), _holdingItem); 
                        }
                        
                        kilnView.GetZDO().Set("queued", queued + amountToFeed);
                        
                        kiln.m_oreAddedEffects.Create(transform.position, Quaternion.identity);
